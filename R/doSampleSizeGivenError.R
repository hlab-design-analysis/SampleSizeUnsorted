	doSampleSizeGivenError<-function(x, e, error_type)
		...
		sprintfnumber<-max(nchar(c(0.02,0.025,0.05,0.07,0.10)))-2

		if (error_type=="Percent")
			{
			for (i in 1:length(e)) x[, paste0("n_", sprintf(paste0("%0.",sprintfnumber,"f"),e[i])):=ceiling(1/bucWeightmean_obs^2*(1.96*sqrt(sppPercWeight_s2)/e[i])^2), by=.(lanID, sp)]
			}
