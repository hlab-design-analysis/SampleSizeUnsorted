#' Max of sample size estimates
#'
#' This function calculates the max of sample size estimates obtained for a set of landings.
#'
...
#' max sample size by error margin and species)
...
#'
#' @details A minimum of 2 samples is needed to calculate a variance. If group == NULL returns
#' the largest sample size of species across all landings. If group == "lanID" returns
#' the largest sample size of species in each landing. If the dataset contains
...
#' summariseMax(x = dat, group="sp")
#' # compare with
#' summariseMax(x = dat, group="sp", min_n=0)
#' }

summariseMax<-function(x, group=c("sp"), min_n=2){
...
# calculates the max
out<-res[, lapply(.SD, max), .SDcols=colnames(x)[grepl(colnames(x), pat="^n_")], by=group]
if (!is.null(group)) {
cols2<-c(group, "lanID")
